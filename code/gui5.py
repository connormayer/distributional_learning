...
CLUSTERER_ARGS = (("Stem of Files",""),("Output File",""),("V Scalar","1"),("Constrain Partition","False"),("Constrain PCS","False"))
...
class Clusterer_tk():
    ...
    def run(self):
        ...
        if (self.constrain_partition == "False"):
            self.constrain_partition = False
        else:
            self.constrain_partition = True
        if (self.constrain_pcs == "False"):
...
# Argument 1 file for Vertex Model Builder
...
arg_path_ent = tk.Entry(master=vmb_frame,width=30)
...
outdir_ent = tk.Entry(master=vmb_frame,width=30)
...
# Argument 1
file_name_ent = tk.Entry(master=clusterer_frame,width=30)
...
output_file_ent = tk.Entry(master=clusterer_frame)
